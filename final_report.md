...
2. **Empirical Validation.** We conducted 138 experiments across three scenarios—eviction stress tests (69 tests), stampede scenarios (60 tests), and realistic multi-phase workloads (9 tests)—measuring hit ratio, throughput, backend load, and tail latency.
...
- `EvictionStrategy` interface: Pluggable LRU or SIEVE
- `RefreshStrategy` interface: Pluggable Naive, Coalescing, or PER
- `CacheEntry` record: Stores `value`, `expiryTime` (millis), `delta` (computation time in nanos)
...
**File:** `SieveEvictionStrategy.java`
...
**File:** `ProbabilisticEarlyRefreshStrategy.java`
...
**File:** `CoalescingRefreshStrategy.java`
...
**Metrics:** Hit Ratio = `(Total Requests - Backend Requests) / Total Requests`
...
**Tests:** 8 (4 skew levels × 2 modes: LRU+Naive, SIEVE+PER). We also ran a NoCache baseline at 80/20 to confirm caching benefits.
...
This project implemented and empirically validated SIEVE (NSDI'24) and PER (VLDB'15) in a Java caching middleware, conducting 138 experiments across eviction, stampede, and realistic workload scenarios.
